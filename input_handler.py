...
if config.IS_WINDOWS:
    ...
    _PYNPUT_SPECIALS = {
        "`": pk.Key.grave, "grave": pk.Key.grave, "tab": pk.Key.tab, "caps_lock": pk.Key.caps_lock,
...
def hold_key(key: str) -> None:
    if config.IS_WINDOWS: _win_send_scan(SCAN_CODES[key], False, key in EXTENDED_KEYS)
    else: _ui.write(EV_KEY, EVDEV_KEY_MAP[key], 1); _ui.syn()

def release_key(key: str) -> None:
    if config.IS_WINDOWS: _win_send_scan(SCAN_CODES[key], True, key in EXTENDED_KEYS)
...
def hotkey_listener(bot: 'FishingBot'):
    if config.IS_WINDOWS:
        ...
        # FIXED: Listener context manager must be OUTSIDE the on_press function
